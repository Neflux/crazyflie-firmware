  ...
  while(!leave) {
    ...
    switch(step->action)
    {
      ...
      default:  //The step is a LED action and a time
        ...
        ledSet(LED_RED_R, right_red_value);
        ledSet(LED_RED_L, left_red_value);
        ledSet(LED_GREEN_L, left_green_value);
        ledSet(LED_GREEN_R, right_green_value);
        ledSet(LED_BLUE_L, left_blue_value);
        // ledSet(LED_BLUE_NRF, left_blue_value);
